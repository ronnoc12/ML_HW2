...
def linear_predict(data, model):
    ...
    classPredictions = model['weights'].T.dot(data)
    classPredictions = classPredictions.argmax(0)
...
def perceptron_update(data, model, params, label):
    ...
    #dmodel, numClasses = model.shape

    for t in range(model.shape): #might need to be num classes? 
        for i in range(data):
            prediction = sign(model['weights'].dot(data))

            if (prediction != label)
            {
                model['weights'] += prediction.dot(data)
            }
